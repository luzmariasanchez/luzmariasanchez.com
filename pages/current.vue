<template>
  <div class="w-full container mx-auto">
    ...
    <template v-else>
      ...
      <Pagination :pathName="'current'" :currentPage="+currentPagination" :totalPage="+totalPagination"></Pagination>
    </template>
  </div>
</template>
...
export default {
  ...
  async asyncData(context) {
    let page, error;
    try {
      page = await context.$content(context.i18n.locale, 'pages', 'current').fetch();
    } catch (err) {
      error = err;
    }
